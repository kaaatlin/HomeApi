...
namespace HomeApi.Controllers
{
    ...
    public class RoomsController : ControllerBase
    {
        ...
        [HttpPut]
        [Route("{id}")]
        public async Task<IActionResult> Put([FromRoute] Guid id, [FromBody] UpdateRoomRequest request)
        {
            var room = await _repository.GetRoomById(id);
            if (room == null)
                return StatusCode(400, $"Ошибка: Комната с индентификатором {id} не подключена. Сначала подключите комнату!");
            ...
            return StatusCode(200, $"Комната обновлена! Имя - {room.Name}, Площадь - {room.Area}, Газ подключен - {room.GasConnected}, Мощность - {room.Voltage}");
        }
    }
}
